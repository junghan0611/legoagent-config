...
이 앱은 효율을 위한 도구가 아니라 **레고에이전트**의 첫 흐름이다.

- **자동 폴링 없음**. 배터리/허브 상태는 아이가 버튼을 눌렀을 때만 업데이트된다.
- 5초마다 갱신되는 숫자는 어른의 리듬이다. 아이는 *"눌렀더니 허브가 응답했다"* 한 번에 감동한다.
- notify는 항상 켜져 있어 허브가 자발적으로 보내는 줄(`emit("ok ..."`, `emit("? ..."`)은 즉시 화면에 흐른다 — 그 흐름은 아이의 행동이 만든 것이지, 앱이 만든 게 아니다.
- 이 흐름이 몸에 새겨져야 나중에 어떤 고도화된 존재(에이전트/자동화/추론)를 끼워 넣어도 거부감이 없다.
